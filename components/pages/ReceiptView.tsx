    ...
    if (!receipt) {
        return (
            <div className="bg-white p-8 rounded-lg shadow-md text-center">
                <h2 className="text-xl font-semibold">No Receipts Found</h2>
        ...
        <div className="max-w-2xl mx-auto">
             <div className="flex justify-between items-center mb-4 no-print">
                ...
                <div className="flex gap-2">
                    {!isIncome && (
                        <button onClick={() => navigate(`/receipts/edit/${receipt.id}`)} className="bg-white border border-slate-300 text-slate-800 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-slate-100 transition-colors">Edit</button>
                    )}
                ...
                <div className="p-8 md:p-10" id="invoice-content">
                    <header className="flex justify-between items-start mb-10">
                        ...
                        <div className="text-right">
                            <h2 className="text-3xl font-bold uppercase text-slate-400">Receipt</h2>
                    ...
                    <section className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        <div>
                            <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">{isIncome ? 'Received From' : 'Paid To'}</h3>
                            <p className="font-bold text-slate-800">{receipt.from}</p>
                        </div>
                        <div className="space-y-3">
                             <div>
                                <p className="text-sm font-semibold text-slate-500">Transaction Date</p>
                    ...
                    {receipt.photoUrl && (
                        <section className="mb-8">
                            <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Photo Receipt</h3>
